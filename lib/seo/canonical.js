...
const FALLBACK_SITE_URL = "https://www.orionapexcapital.com";
const BASE_URL = process.env.NEXT_PUBLIC_SITE_URL || FALLBACK_SITE_URL;
...
export function getCanonical(path) {
  ...
  // Normalize path: ensure single leading slash, remove trailing slashes (except root), and collapse double slashes
  let normalizedPath = path.startsWith('/') ? path : `/${path}`;
  normalizedPath = normalizedPath.replace(/\/{2,}/g, '/'); // collapse multiple slashes
